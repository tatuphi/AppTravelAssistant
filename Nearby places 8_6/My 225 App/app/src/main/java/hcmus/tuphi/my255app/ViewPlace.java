...
import hcmus.tuphi.my255app.Model.Location;
...
public class ViewPlace extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Double latitude=Double.parseDouble(Common.currentResult.getGeometry().getLocation().getLat());
        Double longtitude=Double.parseDouble(Common.currentResult.getGeometry().getLocation().getLng());
            String strAdd = "";
            Geocoder geocoder = new Geocoder(this, Locale.getDefault());

        try {
                List<Address> addresses = geocoder.getFromLocation(latitude, longtitude, 1);
//            String phonenum = addresses.get(0).getPhone(); // If any additional address line present than only, check with max available address lines by getMaxAddressLineIndex()
//             place_phone.setText(phonenum);
            if (addresses != null) {
                ...
                .enqueue(new Callback<PlaceDetail>() {
                    ...
                    public void onResponse(Call<PlaceDetail> call, Response<PlaceDetail> response) {
                     mPlace=response.body();
                        ...
                        rating.setText("("+Common.currentResult.getRating()+")");
                    }
